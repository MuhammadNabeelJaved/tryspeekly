...
router.route('/admin/comments/:id')
  .patch(authenticate, authorizeBlog, logActivity('update', 'blog-comment', (req) => ({ resourceId: req.params.id, details: `Comment status → ${req.body.status ?? ''}` })), updateBlogCommentStatus)
  .delete(authenticate, authorizeBlog, logActivity('delete', 'blog-comment', (req) => ({ resourceId: req.params.id, details: 'Deleted comment' })), deleteBlogComment)
