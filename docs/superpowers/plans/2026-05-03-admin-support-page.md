# Admin Support Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `AdminSupport.tsx` from scratch as a full support center with stats, ticket conversation management, and FAQ/knowledge base CRUD.

**Architecture:** Single component file (`AdminSupport.tsx`) with all state at the root. Stats derived from ticket state. Two tabs: Tickets (split panel) and Knowledge Base (full-width CRUD). No new files except `FAQEntry` type + seed data added to `adminData.ts`.

**Tech Stack:** React 18, TypeScript (strict), Tailwind CSS, Framer Motion, @phosphor-icons/react, localStorage for persistence.
...
|------|--------|----------------|
| `client/src/pages/admin/adminData.ts` | Modify | Add `FAQEntry` interface and `INITIAL_FAQS` constant |
| `client/src/pages/AdminPage.tsx` | Modify | Add `'support'` to `AdminView` union type |
| `client/src/pages/admin/AdminSupport.tsx` | Rewrite | Full support center component |

---

## Task 1: Add FAQEntry type and seed data to adminData.ts

**Files:**
- Modify: `client/src/pages/admin/adminData.ts`

- [ ] **Step 1: Open adminData.ts and locate the end of the type definitions block** (around line 55, after `FinancialAidApp`). Add the `FAQEntry` interface immediately after:
...
- [ ] **Step 2: Add `INITIAL_FAQS` constant at the very end of the file** (after `INITIAL_SUPPORT_TICKETS`):
...
Expected: No errors from `adminData.ts`.

- [ ] **Step 4: Commit**

```bash
git add client/src/pages/admin/adminData.ts
git commit -m "feat(support): add FAQEntry type and INITIAL_FAQS seed data"
```

---

## Task 2: Fix AdminView type in AdminPage.tsx

**Files:**
- Modify: `client/src/pages/AdminPage.tsx` (line 25)

- [ ] **Step 1: Open `AdminPage.tsx`. Find line 25 — the `AdminView` type definition:**
...
Run: `cd client && npx tsc --noEmit`
Expected: No errors. The `view: 'support'` in `NAV_MANAGEMENT` (line 59) will now resolve correctly.

- [ ] **Step 3: Commit**

```bash
git add client/src/pages/AdminPage.tsx
git commit -m "fix(admin): add 'support' to AdminView union type"
```

---

## Task 3: Rewrite AdminSupport.tsx

**Files:**
- Rewrite: `client/src/pages/admin/AdminSupport.tsx`

This is the full component. Replace the entire file contents with the implementation below.

- [ ] **Step 1: Replace the entire contents of `AdminSupport.tsx`**
...
Run: `cd client && npx tsc --noEmit`
Expected: No errors in `AdminSupport.tsx`, `adminData.ts`, or `AdminPage.tsx`.

- [ ] **Step 3: Start dev server and manually verify**

Run: `cd client && npm run dev`

Open: `http://localhost:5173/admin` → log in with `admin123` → click **Support** in the sidebar.

Check these scenarios:
1. **Stats bar** — 4 cards visible with correct counts (Open: 1, Pending: 1, Closed: 1, Avg Response: not N/A)
2. **Tickets tab** — ticket list shows 3 tickets; clicking one opens conversation pane
3. **Search** — typing "Ahmed" filters to one ticket
4. **Status/Priority dropdowns** — changing status on the active ticket updates both the header select and the badge in the list
5. **Send message** — type a reply, hit send; message appears right-aligned in violet; input clears; ticket status does NOT change automatically
6. **Mobile** — resize browser to < 1024px; ticket list fills screen; selecting a ticket slides it out and shows conversation; back button returns to list
7. **Knowledge Base tab** — click KB tab; 4 seed FAQs displayed
8. **Add FAQ** — click "Add FAQ", fill in question + answer, save; new card appears
9. **Edit FAQ** — click pencil on a card; form pre-fills; save updates the card
10. **Delete FAQ** — click trash; card removed immediately
11. **Dark mode** — toggle dark mode; all surfaces, badges, and text render correctly

- [ ] **Step 4: Commit**

```bash
git add client/src/pages/admin/AdminSupport.tsx
git commit -m "feat(support): rewrite admin support page with stats, tickets, and knowledge base"
```
...
**Spec coverage:**
- [x] Stats bar — 4 cards (Open, Pending, Closed, Avg Response): Task 3, `SupportStats`
- [x] Tickets tab with left/right split panel: Task 3, root component
- [x] Ticket list with search + status/priority filters: Task 3
- [x] Conversation pane with message thread, auto-scroll, reply input: Task 3
- [x] Status and priority update via dropdowns in conversation header: Task 3
- [x] Mobile: ticket list and conversation toggle full-screen: Task 3 (translate-x classes)
- [x] Knowledge Base tab: Task 3, `KnowledgeBaseTab`
- [x] FAQ CRUD — add, edit, delete: Task 3, `KnowledgeBaseTab`
- [x] FAQ empty state: Task 3
- [x] localStorage persistence for both tickets and FAQs: Task 3
- [x] FAQEntry type + INITIAL_FAQS: Task 1
- [x] AdminView type fix: Task 2
- [x] `newStatus` bug fixed: Task 3 — `handleSendMessage` no longer references it

**Placeholder scan:** No TBDs, TODOs, or vague steps. All code blocks are complete.

**Type consistency:**
- `SupportTicket['status']` used consistently in `handleUpdateStatus` and the select onChange
- `SupportTicket['priority']` used consistently in `handleUpdatePriority`
- `FAQEntry` imported from `adminData` in Task 3, defined in Task 1
- `INITIAL_FAQS` exported in Task 1, imported in Task 3
- `_store` prefixed to suppress unused-variable warning (AdminSupport receives `store` prop for API consistency but doesn't use it yet)
