# Hero Creative Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the two-column hero (text + stock photo) with an immersive full-bleed gradient orb background and a bold centered single-column layout.

**Architecture:** Single file change — `client/src/components/Hero.tsx` is completely rewritten. The Typewriter component and all Framer Motion variant types are preserved. The stock image, right column, and decorative circle borders are removed. Three animated gradient orbs replace the current two-blob background. Two glassmorphism stat cards are repositioned as absolute-positioned overlays. A scroll indicator is added at the bottom.

**Tech Stack:** React, TypeScript, Tailwind CSS v4, Framer Motion, Phosphor Icons

---

## Files Modified

- `client/src/components/Hero.tsx` — full rewrite, same exports

---

### Task 1: Rewrite Hero with Immersive Background + Centered Layout

**Files:**
- Modify: `client/src/components/Hero.tsx`

- [ ] **Step 1: Replace the entire contents of `client/src/components/Hero.tsx` with the following**
...
Expected: `✓ built in Xs` with no TypeScript errors. If errors appear, check imports — `CaretDown` must be imported from `@phosphor-icons/react` and `VideoCamera`, `ChatCircleDots` must be removed since they are no longer used.

- [ ] **Step 3: Commit**

```bash
git add client/src/components/Hero.tsx
git commit -m "feat(hero): immersive gradient orb background, centered single-column layout"
```
...
Check:
- Three gradient orbs visible as soft background color halos
- "Master English" headline is very large (nearly full-width on desktop)
- Typewriter word appears below headline in gradient text with highlight bar
- Badge, CTA buttons, social proof all centered
- Two floating glassmorphism cards visible on left and right sides (desktop only)
- "Scroll" indicator with animated caret at bottom center
- No stock image visible

- [ ] **Step 3: Toggle dark mode and verify**

Check:
- Orbs glow more intensely against dark background
- All text readable (white headline, gray subtext)
- Floating cards use `dark:bg-slate-900/80` glass style
- Scroll indicator is `dark:text-gray-600`

- [ ] **Step 4: Check mobile (resize to 375px)**

Check:
- Headline wraps cleanly, still large
- Floating cards are hidden (`hidden lg:block`)
- Scroll indicator hidden (`hidden sm:flex`)
- CTAs remain side-by-side (`flex-row flex-wrap`)
