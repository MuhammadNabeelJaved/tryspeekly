...
import {
  Bell, CheckCircle, Clock, Info, Warning, CreditCard, UserPlus, ShieldCheck,
  MagnifyingGlass, X,
...
function getIcon(type: Notification['type'], severity: Notification['severity']) {
  ...
  if (type === 'user') return <UserPlus size={20} className="text-blue-500" />
  if (type === 'security') return <ShieldCheck size={20} className="text-violet-500" />
...
function getBg(type: Notification['type'], severity: Notification['severity']) {
  ...
  if (type === 'user') return 'bg-blue-50 dark:bg-blue-950/30'
  if (type === 'security') return 'bg-violet-50 dark:bg-violet-950/30'
    ...
    <div className="p-4 sm:p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 dark:text-neutral-500 mt-0.5">Stay updated with platform activity and alerts.</p>
        </div>
        {unreadCount > 0 && (
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="bg-white dark:bg-neutral-900 border border-slate-100 dark:border-neutral-800 rounded-2xl p-4 animate-pulse">
                <div className="flex gap-3">
                  <div className="w-10 h-10 bg-slate-100 dark:bg-neutral-800 rounded-xl" />
                  <div className="flex-1 space-y-2">
                    <div className="h-3 bg-slate-100 dark:bg-neutral-800 rounded w-1/3" />
          ...
          <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-2xl p-10 text-center">
            <div className="w-12 h-12 bg-slate-50 dark:bg-neutral-800 rounded-xl flex items-center justify-center mx-auto mb-3 text-slate-300 dark:text-neutral-700">
              <Bell size={24} />
            </div>
            <h3 className="text-sm font-black text-slate-900 dark:text-white">No notifications found</h3>
            ...
            {activeFilterCount > 0 && (
              <button onClick={clearFilters} className="mt-3 text-xs font-bold text-violet-600 dark:text-violet-400 hover:underline">
                Clear all filters
              </button>
            )}
          </div>
          ...
          filtered.map((notif, index) => (
            <motion.div key={notif._id}
              ...
              className={`group bg-white dark:bg-neutral-900 border rounded-2xl p-4 transition-all hover:shadow-sm ${notif.read ? 'border-slate-100 dark:border-neutral-800' : 'border-violet-200 dark:border-violet-900/50'}`}>
              <div className="flex gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${getBg(notif.type, notif.severity)}`}>
                  {getIcon(notif.type, notif.severity)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h4 className={`text-sm ${notif.read ? 'font-semibold' : 'font-black'} text-slate-900 dark:text-white`}>
                        {notif.title}
                        {!notif.read && <span className="ml-2 w-1.5 h-1.5 rounded-full bg-violet-600 inline-block" />}
                      </h4>
                      <p className="text-xs text-slate-500 dark:text-neutral-400 mt-0.5 leading-relaxed">{notif.message}</p>
                    </div>
                    {!notif.read && (
                      <button onClick={() => markAsRead(notif._id)}
                        className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 text-slate-400 hover:text-violet-600 dark:hover:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-900/20 rounded-lg"
                        title="Mark as read">
                        <CheckCircle size={16} />
                      </button>
                    )}
                  ...
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-2">
                    <div className="flex items-center gap-1 text-[10px] font-semibold text-slate-400 dark:text-neutral-600">
                      <Clock size={12} />
                      {timeAgo(notif.createdAt)}
                    </div>
                    <span className="text-slate-200 dark:text-neutral-700">·</span>
                    <span className="text-[10px] font-bold text-violet-600 dark:text-violet-400 uppercase tracking-wider">{notif.type.replace('_', ' ')}</span>
                    <span className="text-slate-200 dark:text-neutral-700">·</span>
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${notif.severity === 'high' ? 'text-rose-500' : notif.severity === 'medium' ? 'text-amber-500' : 'text-slate-400 dark:text-neutral-500'}`}>
                      {notif.severity}
                    </span>
                    <span className="text-slate-200 dark:text-neutral-700">·</span>
